...
.theme-toggle { position: fixed; top: 20px; left: 40px; z-index: 6; display: flex; background: rgba(0,0,0,0.35); border-radius: 20px; padding: 4px; gap: 4px; }
.theme-toggle-btn { padding: 6px 14px; border: none; border-radius: 16px; cursor: pointer; font-size: 14px; font-weight: 600; background: transparent; color: #fff; opacity: 0.6; transition: all 0.15s ease; }
.theme-toggle-btn.active { background: #fff; color: #222; opacity: 1; }
.action-panel { position: fixed; top: 20px; right: 40px; display: flex; flex-direction: column; gap: 14px; z-index: 6; }
.btn-large { padding: 18px 38px; border: none; border-radius: 14px; font-weight: 700; font-size: 18px; cursor: pointer; color: #fff; transition: transform 0.15s ease, opacity 0.15s ease; box-shadow: 0 6px 16px rgba(0,0,0,0.3); }
.btn-large:hover { transform: translateY(-2px); }
.btn-large:disabled { opacity: 0.5; cursor: not-allowed; }
.btn-newgame { background: var(--accent); }
.btn-pause { background: #dc2626; }
.btn-resume { background: #16a34a; }
.timer-panel { position: fixed; left: 40px; top: 50%; transform: translateY(-50%); width: 150px; height: 260px; background: var(--panel-bg); border-radius: 12px; overflow: hidden; display: flex; flex-direction: column; box-shadow: 0 8px 24px rgba(0,0,0,0.3); z-index: 5; }
...
.black-clock { background: #222; }
.white-clock { background: #e5e5e5; color: #111; }
.clock-box.active.black-clock { background: #444; }
.clock-box.active.white-clock { background: #fff; }
.clock-time { font-size: 30px; font-weight: bold; margin-top: 4px; }
.clock-divider { height: 2px; background: #555; }
.move-history-panel { position: fixed; right: 40px; top: 50%; transform: translateY(-50%); width: 200px; max-height: 400px; background: var(--panel-bg); border-radius: 12px; padding: 14px; color: #fff; box-shadow: 0 8px 24px rgba(0,0,0,0.3); z-index: 5; display: flex; flex-direction: column; }
.panel-title { font-weight: 700; margin-bottom: 8px; text-align: center; letter-spacing: 0.5px; }
.move-list { overflow-y: auto; flex: 1; font-size: 14px; }
.move-row { display: flex; gap: 8px; padding: 3px 0; border-bottom: 1px solid rgba(255,255,255,0.08); }
.move-number { width: 24px; opacity: 0.7; }
.move-white { flex: 1; }
.move-black { flex: 1; opacity: 0.85; }
.center-column { display: flex; flex-direction: column; align-items: center; position: relative; z-index: 2; padding-top: 40px; }
.captured-row { display: flex; gap: 6px; min-height: 34px; flex-wrap: wrap; justify-content: center; max-width: 640px; margin: 6px 0; }
.captured-piece { font-size: 24px; opacity: 0.85; }
.board-grid { display: grid; grid-template-columns: repeat(8, 80px); grid-template-rows: repeat(8, 80px); border: 4px solid var(--board-border); border-radius: 12px; overflow: hidden; box-shadow: 0 12px 40px rgba(0,0,0,0.35); transition: filter 0.2s ease; }
.board-disabled { pointer-events: none; filter: grayscale(0.3) brightness(0.85); }
.status-text { margin-top: 14px; font-weight: 700; font-size: 18px; color: #444; text-align: center; }
.status-gameover { color: #d32f2f; }
    ...
    const pieces = useMemo(
        () =>
            Array.from({ length: 40 }).map((_, i) => ({
                ...
                color: ["#eb2592", "#fbbf24", "#3b82f6", "#10b981", "#f97316"][i % 5],
